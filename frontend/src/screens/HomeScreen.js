...
export default function HomeSearch({ navigation }) {
    ...
    function handleFilter(searchTerm) {
        if (searchTerm != undefined || (searchTerm == undefined && seCity != null)) {
            ...
            if (priceRange) {
                if (priceRange == 1) {
                    newData = newData.filter((item) => item.expenses < 1000000)
                } else if (priceRange == 2) {
                    newData = newData.filter(
                        (item) => item.expenses < 2500000 && item.expenses >= 1000000
                    )
                } else if (priceRange == 3) {
                    newData = newData.filter(
                        (item) => item.expenses < 5000000 && item.expenses >= 2500000
                    )
                } else if (priceRange == 4) {
                    newData = newData.filter((item) => item.expenses >= 5000000)
                }
            }
